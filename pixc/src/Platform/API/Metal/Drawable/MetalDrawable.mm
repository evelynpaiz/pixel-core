...
void* MetalDrawable::GetOrCreateRenderPipelineState(const std::shared_ptr<FrameBuffer>& framebuffer)
{
    ...
    @autoreleasepool
    {
        ...
        for (size_t i = 0; i < colorAttachmentCount; ++i)
        {
            ...
            utils::textures::mtl::ToMetalPixelFormat(framebuffer->GetColorAttachment((uint32_t)i)->GetSpecification().Format) :
            MTLPixelFormatRGBA8Unorm;
        }
        // Configure depth attachment format
        if(!framebuffer || utils::graphics::IsBufferActive(framebuffer->GetEnabledTargets(), RenderTargetMask::Depth))
        {
